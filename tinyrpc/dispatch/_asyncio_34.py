...
class AsyncioRPCDispatcher(RPCDispatcher):
    ...
    def dispatch(self, request):
        ...
        if hasattr(request, 'create_batch_response'):
            ...
            results = []
            for req in request:
                res = yield from self._dispatch(req)
                results.append(res)
            ...
            return (yield from self._dispatch(request))
    ...
    def validate_parameters(self, method, args, kwargs):
        ...
        if asyncio.iscoroutinefunction(method):
            # asyncio.coroutine decorated functions 
            # don't preserve the wrapped function signature
            # so getcallargs won't raise TypeError
            # but we can access the wrapped function instead
            assert hasattr(method, "__wrapped__"), \
                "coroutine function does not have __wrapped__ attribute"
